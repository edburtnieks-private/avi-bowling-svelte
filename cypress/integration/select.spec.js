...
describe('checkbox', () => {
  beforeEach(() => {
    ...
    cy.get('[data-cy=caret-icon]').as('caret-icon');
  });
  ...
  it('should show select with changing states', () => {
    ...
    cy.get('@select')
      ...
      .and('have.id', 'select');
    cy.get('@caret-icon').should('not.have.class', 'active');
  ...
  it('should show select with disabled prop', () => {
    ...
    cy.get('@select').should('be.disabled');
    cy.get('@caret-icon').should('have.class', 'disabled');
  });
});
